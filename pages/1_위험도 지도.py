...
def render_header():
    ...
    st.markdown("""
    ### 이 페이지는?
    서울시 각 지역의 젠트리피케이션 위험도를 **색상과 원 크기**로 직관적으로 표현한 지도입니다.

    - 점의 색상: 위험 점수가 높을수록 **빨강**, 낮을수록 **초록**
    - 점의 크기: 위험 점수가 높을수록 **더 큼**
    - 특정 시점의 지역별 위험도를 **한눈에 비교**할 수 있습니다.
    """)
    st.divider()
...
def render_map(df):
    ...
    layer = pdk.Layer(
        ...
        get_fill_color="""
            [255 * FINAL_SCORE, 255 * (1 - FINAL_SCORE), 0, 160]
        """,
        pickable=True,
        auto_highlight=True,
    )
